...
![`有符号`的`上溢出`](./assets/8.svg)

* 那么，`有符号`的`下溢出`，原理就是这样的：

![`有符号`的`下溢出`](./assets/9.svg)
